...
import com.kopihao.drawer.tksm.R;
import com.view.plugin.tksm.TKScrollDetectors.ScrollDetector;
 ...
 *
 */
public class TKSlideMenu extends ViewGroup {
	...
	public final static Interpolator ACCELERATE_INTERPOLATOR = new AccelerateDecelerateInterpolator();
	public final static Interpolator ANTICIPATE_INTERPOLATOR = new AnticipateInterpolator();
	public final static Interpolator ANTICIPATEOVERSHOOT_INTERPOLATOR = new AnticipateOvershootInterpolator();
	public final static Interpolator BOUNCE_INTERPOLATOR = new BounceInterpolator();
	public final static Interpolator DECELERATE_INTERPOLATOR = new DecelerateInterpolator();
	public final static Interpolator LINEAR_INTERPOLATOR = new LinearInterpolator();
	public final static Interpolator OVERSHOOT_INTERPOLATOR = new OvershootInterpolator();
	...
	public TKSlideMenu(Context context) {
		this(context, null);
	}

	public TKSlideMenu(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public TKSlideMenu(Context context, AttributeSet attrs, int defStyle) {
		...
		if (!this.isInEditMode()) {
			...
			TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TKSlideMenu, defStyle, 0);

			// Set the shadow attributes
			setPrimaryShadowWidth(a.getDimension(R.styleable.TKSlideMenu_primaryShadowWidth, 30));
			setSecondaryShadowWidth(a.getDimension(R.styleable.TKSlideMenu_secondaryShadowWidth, 30));

			Drawable primaryShadowDrawable = a.getDrawable(R.styleable.TKSlideMenu_primaryShadowDrawable);
			...
			Drawable secondaryShadowDrawable = a.getDrawable(R.styleable.TKSlideMenu_secondaryShadowDrawable);
			...
			int interpolatorResId = a.getResourceId(R.styleable.TKSlideMenu_interpolator, -1);
			setInterpolator(-1 == interpolatorResId ? DEFAULT_INTERPOLATOR
					: AnimationUtils.loadInterpolator(context, interpolatorResId));

			mSlideDirectionFlag = a.getInt(R.styleable.TKSlideMenu_slideDirection,
					FLAG_DIRECTION_LEFT | FLAG_DIRECTION_RIGHT);

			setEdgeSlideEnable(a.getBoolean(R.styleable.TKSlideMenu_edgeSlide, false));
			setEdgetSlideWidth(a.getDimensionPixelSize(R.styleable.TKSlideMenu_edgeSlideWidth, 100));
			a.recycle();
	...
	public void addView(View child, int index, android.view.ViewGroup.LayoutParams params) {
		if (!(params instanceof LayoutParams)) {
			throw new IllegalArgumentException(
					"The parameter params must a instance of com.aretha.TKSlideMenu.TKSlideMenu$LayoutParams");
		}
	...
	/**
	 * Set animation interpolator when TKSlideMenu scroll
	 *
	 * @param interpolator
	 */
	...
	public void setSlideMode(int slideMode) {
		if (isAttacthedInContentView()) {
			throw new IllegalStateException("JFSlideMenu must be the root of layout");
		}
	...
	/**
	 * Indicate user can only open TKSlideMenu from the edge
	 *
	 * @return
	 */
	...
	/**
	 * Set edge slide width next left and right side of TKSlideMenu
	 *
	 * @param width
	 */
	...
	/**
	 * Indicate this TKSlideMenu is open
	 *
	...
	private void setCurrentOffset(int currentOffset) {
		...
		final int currentContentOffset = mCurrentContentOffset = Math.min(
				(slideDirectionFlag & FLAG_DIRECTION_RIGHT) == FLAG_DIRECTION_RIGHT ? mContentBoundsRight : 0,
				Math.max(currentOffset,
						(slideDirectionFlag & FLAG_DIRECTION_LEFT) == FLAG_DIRECTION_LEFT ? mContentBoundsLeft : 0));
		...
		if (STATE_SCROLL == mCurrentState || isOpen()) {
			if (mScroller.computeScrollOffset()) {
				...
				setCurrentState(mCurrentContentOffset == 0 ? STATE_CLOSE
						: (mCurrentContentOffset > 0 ? STATE_OPEN_LEFT : STATE_OPEN_RIGHT));
			}
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int index = 0; index < count; index++) {
			...
			needRemeasure |= (MeasureSpec.EXACTLY != measureHeightMode
					&& android.view.ViewGroup.LayoutParams.MATCH_PARENT == layoutParams.height);
			switch (layoutParams.role) {
			case LayoutParams.ROLE_CONTENT:
			...
			case LayoutParams.ROLE_SECONDARY_MENU:
				measureChildWithMargins(child, widthMeasureSpec, 0,
						slideMode == MODE_SLIDE_WINDOW ? MeasureSpec.makeMeasureSpec(measureHeight - statusBarHeight,
		...
		// we know the exactly size of TKSlideMenu, so we should use the new
		// size
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int index = 0; index < count; index++) {
			...
			case LayoutParams.ROLE_SECONDARY_MENU:
				...
				child.layout(r - l - paddingRight - measureWidth, statusBarHeight + paddingTop, r - l - paddingRight,
						statusBarHeight + paddingTop + measureHeight);
				break;
			default:
				continue;
		...
		if (v instanceof ViewGroup) {
			...
			for (int index = 0; index < childCount; index++) {
				...
				if (x + scrollX >= left && x + scrollX < child.getRight() && y + scrollY >= top
						...
						&& (TKScrollDetectors.canScrollHorizontal(child, dx)
	...
	protected final boolean canScrollVertically(View v, int dy, int x, int y) {
		if (v instanceof ViewGroup) {
			...
			for (int index = 0; index < childCount; index++) {
				...
				if (x + scrollX >= left && x + scrollX < child.getRight() && y + scrollY >= top
						&& y + scrollY < child.getBottom() && View.VISIBLE == child.getVisibility()
						&& (TKScrollDetectors.canScrollVertical(child, dy)
	...
	/**
	 * Add view role for {@link #JFSlideMenu}
	 *
	 * @author Tank
	 *
	 */
	public static class LayoutParams extends MarginLayoutParams {
		...
		public LayoutParams(Context context, AttributeSet attrs) {
			...
			TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TKSlideMenu, 0, 0);

			final int indexCount = a.getIndexCount();
			for (int index = 0; index < indexCount; index++) {
				int aIndex = a.getIndex(index);
				if (aIndex == R.styleable.TKSlideMenu_layout_role) {
					role = a.getInt(R.styleable.TKSlideMenu_layout_role, -1);
				}
			}

			switch (role) {
			// content should match whole JFSlideMenu
			case ROLE_CONTENT:
				width = MATCH_PARENT;
				height = MATCH_PARENT;
				break;
	...
	public interface OnSlideStateChangeListener {
		 ...
		 *
		 * @param slideState
		 *            {@link TKSlideMenu#STATE_CLOSE},
		 *            {@link TKSlideMenu#STATE_OPEN} ,
		 *            {@link TKSlideMenu#STATE_DRAG},
		 *            {@link TKSlideMenu#STATE_SCROLL}
		 */
	...
	public interface OnContentTapListener {
		public void onContentTap(TKSlideMenu slideMenu);
	}

	public void configure(TKSlideMenuConfig pConfig) {
		// INFO Slide Shadow
		this.setPrimaryShadowWidth(pConfig.getShadowLeft());
		this.setSecondaryShadowWidth(pConfig.getShadowRight());

		// INFO Slide Animation
		this.setInterpolator(pConfig.getInterpolator());

		// INFO Slide Area
		if (pConfig.isSlideContent()) {
			this.setSlideMode(TKSlideMenu.MODE_SLIDE_CONTENT);
		} else {
			this.setSlideMode(TKSlideMenu.MODE_SLIDE_WINDOW);
		}

		int slideDirectionFlag = this.getSlideDirection();

		if (pConfig.isLockLeft()) {
			// INFO Disallow Swipe Left Gesture
			slideDirectionFlag &= ~TKSlideMenu.FLAG_DIRECTION_LEFT;
		} else {
			// INFO Allow Swipe Left Gesture
			slideDirectionFlag |= TKSlideMenu.FLAG_DIRECTION_LEFT;
		}

		if (pConfig.isLockRight()) {
			// INFO Allow Swipe Right Gesture
			slideDirectionFlag &= ~TKSlideMenu.FLAG_DIRECTION_RIGHT;
		} else {
			// INFO Disallow Swipe Right Gesture
			slideDirectionFlag |= TKSlideMenu.FLAG_DIRECTION_RIGHT;
		}
		this.setSlideDirection(slideDirectionFlag);

		// INFO Slide sensitivity
		if (pConfig.getSlideSensitivity() == 0) {
			this.setEdgeSlideEnable(false);
		} else {
			this.setEdgeSlideEnable(true);
			this.setEdgetSlideWidth(80);
		}
	}
